...
public class TreeCommand extends ObjectCommand<PermissionObject> {
    ...
    public void execute(CommandSender sender, PermissionObject object, String[] arguments) {
        ...
        sender.sendMessage(Messages.OBJECT_GROUP_TREE_HEADER, new DescribedHashVariableSet()
                .add("type",object.getType().getName().toLowerCase())
                .add("object",object));

        builder.setHeaderPrinter(scope1
                -> sender.sendMessage(Messages.OBJECT_GROUP_TREE_SCOPE, new DescribedHashVariableSet()
                .add("scope", scope1)
                .add("spaces",buildSpaced(scope1.getLevel()))));

        builder.setDataPrinter((scope, entry) -> sender.sendMessage(Messages.OBJECT_GROUP_TREE_ENTRY, new DescribedHashVariableSet()
                .add("entry", entry)
                .add("scope", scope)
